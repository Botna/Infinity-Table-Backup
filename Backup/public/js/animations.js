...
if($('div[class="span7"]').find('h3').text() == 'Color Wave')
		{
			//hide the brightness stuff and return a slighty different tinycolor
			$('div[class="config color"]').find('p').remove();
			$('div[class="config color"]').find('input[class="lightness"]').remove();
}
...
if($('div[class="span7"]').find('h3').text() == 'Color Wave')
	...
	else if($(element).data('type') == 'waveColor'){ 
               return tinycolor({
                        h: parseInt($(element).find('input.hue').val()),
                        s: 100,
                        l: 50 }).toRgb();
